...
pub async fn auth_arca(
    ...
    let expir_str = get_xml_tag(&response, "expirationTime").ok_or(SoapFault::new(
        "parseError",
        "No se encontro expirationTime en la respuesta de afip",
    ))?;
    let token = get_xml_tag(&response, "token").ok_or(SoapFault::new(
        "parseError",
        "No se encontro token en la respuesta de afip",
    ))?;
    let sign = get_xml_tag(&response, "sign").ok_or(SoapFault::new(
        "parseError",
